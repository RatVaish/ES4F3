...
int main()
{
	...
	xil_printf("\r\n=== Easter Egg System Ready ===\r\n");
	xil_printf("Flip switches to activate: sw0 ON -> sw1 ON -> sw0 OFF -> sw2 ON\r\n");
	xil_printf("(Menu disabled for testing - just watching switches now)\r\n\r\n");

	// SIMPLIFIED LOOP - Just check switches continuously
	while (1) {
		sequence.update();
		
		// Activate/deactivate based on state, not toggle
		if (sequence.get_state() == SequenceDetector::TRIGGERED) {
